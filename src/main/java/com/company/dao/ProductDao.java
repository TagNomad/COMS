...
public class ProductDao {
    ...
    public boolean insert(Product product) {
        String sql = "INSERT INTO Products (product_name, category_id, description, unit, price, stock_quantity, status) "
                +
                "VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DBConnection.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public boolean update(Product product) {
        String sql = "UPDATE Products SET product_name = ?, category_id = ?, description = ?, " +
                "unit = ?, price = ?, stock_quantity = ?, status = ? WHERE product_id = ?";
        try (Connection conn = DBConnection.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, product.getProductName());
            stmt.setInt(2, product.getCategoryId());
